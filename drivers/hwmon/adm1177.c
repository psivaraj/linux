...
static inline int adm1177_read_raw(struct adm1177_state *st, u8 num, u8 *data)
{
			...
			u32 attr, int channel, long *val)
{
	...
	case hwmon_curr:
		...
		*val = ((105840000 / 4096) * dummy) / st->r_sense_uohm;
		return 0;
	case hwmon_in:
		ret = adm1177_read_raw(st, 3, data);
		if (ret < 0)
		...
		/* convert in millivolts */
		*val = ((6650000 / 4096) * dummy) / st->r_sense_uohm;
		return 0;
	default:
		return -EOPNOTSUPP;
	}
			 ...
			 const struct i2c_device_id *id)
{
	...
	device_property_read_u32(dev, "adi,r-sense-micro-ohms",
				 &st->r_sense_uohm);
	device_property_read_u32(dev, "adi,shutdown-threshold-microamp",
	...
	config = ADM1177_CMD_V_CONT | ADM1177_CMD_I_CONT |
		 (st->vrange_high ? 0 : ADM1177_CMD_VRANGE);
	ret = i2c_smbus_write_byte(st->client, config);
	if (ret)
		return ret;
