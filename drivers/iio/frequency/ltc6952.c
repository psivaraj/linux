		...
		if (i > 15) {
			for (j = 0; j < 7; j++) {
				...
				if (chan->out_divider < ((i + 1) << j))
					break;
				else if (chan->out_divider == ((i + 1) << j))
					return 0;
			}
		}
	}
			    ...
			    long mask)
{
	...
	case IIO_CHAN_INFO_PHASE:
		ltc6952_read(indio_dev, LTC6952_REG(0x0D) +
			     LTC6952_CH_OFFSET(ch->num),
			     &tmp1);
		ltc6952_read(indio_dev, LTC6952_REG(0x0E) +
			     LTC6952_CH_OFFSET(ch->num),
			     &tmp2);
			     ...
			     long mask)
{
	...
	case IIO_CHAN_INFO_FREQUENCY:
		...
		ltc6952_write(indio_dev, LTC6952_REG(0x0C) +
			      LTC6952_CH_OFFSET(ch->num),
			      tmp);
	...
	case IIO_CHAN_INFO_PHASE:
		...
		ltc6952_write_mask(indio_dev, LTC6952_REG(0x0D) +
			      LTC6952_CH_OFFSET(ch->num),
			      LTC6952_DDEL_HIGH_MSK,
			      LTC6952_DDEL_HIGH((tmp & 0xF00) >> 8));
		ltc6952_write(indio_dev, LTC6952_REG(0x0E) +
			      LTC6952_CH_OFFSET(ch->num),
			      LTC6952_DDEL_LOW(tmp));
...
static int ltc6952_setup(struct iio_dev *indio_dev)
{
	...
	for (i = 0; i < 4; i++)
		tmp |= LTC6952_PD(i, 0);
	ltc6952_write(indio_dev, LTC6952_REG(0x03), tmp);
	tmp = 0;
	for (i = 4; i < 8; i++)
		tmp |= LTC6952_PD(i, 0);
	ltc6952_write(indio_dev, LTC6952_REG(0x04), tmp);
	tmp = 0;
	for (i = 8; i < LTC6952_NUM_CHAN; i++)
		tmp |= LTC6952_PD(i, 0);
	ltc6952_write(indio_dev, LTC6952_REG(0x05), tmp);

	/* Program the dividers */
	ltc6952_write_mask(indio_dev, LTC6952_REG(0x06), LTC6952_RD_HIGH_MSK,
			   LTC6952_RD_HIGH((r & 0x300) >> 8));
	ltc6952_write(indio_dev, LTC6952_REG(0x07), LTC6952_RD_LOW(r));
	ltc6952_write(indio_dev, LTC6952_REG(0x08),  LTC6952_ND_HIGH(n >> 8));
	ltc6952_write(indio_dev, LTC6952_REG(0x09), LTC6952_ND_LOW(n));

	/* PLL lock cycle count  */
	ltc6952_write_mask(indio_dev, LTC6952_REG(0x06), LTC6952_LKCT_MSK,
			   LTC6952_LKCT(0x03));

	/* Disable CP Hi-Z */
	ltc6952_write_mask(indio_dev, LTC6952_REG(0x0A), LTC6952_CPRST_MSK,
			   LTC6952_CPRST(0x0));
	...
	for (i = 0; i < st->num_channels; i++) {
		...
		ltc6952_write(indio_dev, LTC6952_REG(0x0F) +
			      LTC6952_CH_OFFSET(chan->num),
